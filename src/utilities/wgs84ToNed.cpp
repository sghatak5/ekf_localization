...
Eigen::Vector3d WGS84toNED::convertToNed(double lat, double lon, double alt){
    ...
    rotationMatrix << -sin(latRefRad) * cos(lonRefRad), -sin(latRefRad) * sin(lonRefRad) , cos(latRefRad),
                        sin(lonRefRad), cos(lonRefRad), 0,
                        -cos(latRefRad) * cos(lonRefRad), -cos(latRefRad) * sin(lonRefRad), -sin(latRefRad);
...
Eigen::Vector3d WGS84toNED::WGS84ToEcef(double lat, double lon, double alt){
    ...
    const double e2 = 1 - ((b*b)/(a*a)); //WGS-84 Earth eccentricity
    ...
    double z = (N * (b*b/a*a) + alt) * sin(latRad);
